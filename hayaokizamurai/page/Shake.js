...
import { selectedTimeState } from '../page/Recoil/selectedTimeState'; // Import the Recoil atom
...
const Shake = ({ soundObject, setIsPlaying, setSoundObject }) => {
  ...
  const [timeData, setTimeData] = useRecoilState(timeDataAtom); // RecoilのuseRecoilStateフックを使って時間データの状態と更新関数を取得
  ...
  const measurement = (time) => {
    ...
    // 結果を出力
    console.log(`端末を振るまでの時間: ${timeDifferenceFormatted}秒`);

     // RecoilのtimeDataAtomにtimeDifferenceFormattedを追加する
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.text}>スマホを侍のように振ってみよう</Text>
    </View>
  );
};
